...
log_dirs_to_try = [
    "/sdcard/Android/data/pth.windyx.plott3r.panel/files/logs",
    "/sdcard/Download/plott3r_logs",
    os.path.join(os.environ.get("HOME", "/tmp"), "logs")
]

log_dir = None
for d in log_dirs_to_try:
    try:
...
logging.info("--- Plott3r Python Engine Started ---")
...
try:
    ...
    found_ocl = None
    import shutil
    for p in opencl_paths:
        if os.path.exists(p):
            try:
                local_ocl = os.path.join(os.environ.get("HOME", "/tmp"), "libOpenCL.so")
                if not os.path.exists(local_ocl) or os.path.getsize(local_ocl) != os.path.getsize(p):
                    shutil.copy2(p, local_ocl)
                os.environ["OPENCV_OPENCL_RUNTIME"] = local_ocl
                found_ocl = local_ocl
                break
            except Exception as e:
                logging.error(f"Failed to copy OpenCL library to local path: {e}")
                os.environ["OPENCV_OPENCL_RUNTIME"] = p
                found_ocl = p
                break
    if found_ocl:
        logging.info(f"Found OpenCL runtime at: {found_ocl}")
    else:
        logging.warning("No OpenCL runtime found in common paths.")
except Exception as e:
    logging.error(f"Error searching for OpenCL: {e}")

import cv2
print("--- OpenCV OpenCL Status ---")
print(f"cv2.ocl.haveOpenCL(): {cv2.ocl.haveOpenCL()}")
print(f"cv2.ocl.useOpenCL(): {cv2.ocl.useOpenCL()}")
print(f"OpenCL runtime: {os.environ.get('OPENCV_OPENCL_RUNTIME')}")
print("----------------------------")
cv2.ocl.setUseOpenCL(True)
logging.info(f"cv2.ocl.haveOpenCL(): {cv2.ocl.haveOpenCL()}")
import matplotlib
...
def resize_image(img: np.ndarray, output_size_px: Tuple[int, int], use_cuda: bool) -> np.ndarray:
    ...
    if cv2.ocl.haveOpenCL():
        try:
            return cv2.resize(cv2.UMat(img), output_size_px, interpolation=cv2.INTER_AREA).get()
        except Exception:
            pass
...
def preprocess_for_edges(img: np.ndarray) -> np.ndarray:
    ...
    if cv2.ocl.haveOpenCL():
        try:
            uimg = cv2.UMat(img)
            blurred = cv2.GaussianBlur(uimg, (5, 5), 0)
            clahe = cv2.createCLAHE(clipLimit=2.2, tileGridSize=(8, 8))
            return clahe.apply(blurred).get()
        except Exception:
            pass
...
def estimate_canny_thresholds(img_gray: np.ndarray) -> Tuple[int, int]:
    if cv2.ocl.haveOpenCL():
        try:
            uimg = cv2.UMat(img_gray)
            gx = cv2.Sobel(uimg, cv2.CV_32F, 1, 0, ksize=3).get()
            gy = cv2.Sobel(uimg, cv2.CV_32F, 0, 1, ksize=3).get()
        except Exception:
...
def load_image_grayscale(filename: str, file_storage=None) -> np.ndarray:
    if file_storage:
        ...
        if len(data) == 0:
            raise ValueError("File is empty or could not be read by Android WebView.")
            
        # 1. Try OpenCV directly (very fast, supports many formats)
        nparr = np.frombuffer(data, np.uint8)
        img = cv2.imdecode(nparr, cv2.IMREAD_GRAYSCALE)
        if img is not None:
            return img
            
        # 2. Try Android Native Decoder via Chaquopy (supports HEIC, WebP, etc.)
        try:
            ...
            if bitmap is not None:
                ...
                png_bytes = bytes(out.toByteArray())
        ...
        # 3. Try PIL as last resort
        try:
            in_memory_file = io.BytesIO(data)
            img_pil = Image.open(in_memory_file).convert("L")
            return np.array(img_pil)
        except Exception as e:
            raise ValueError(f"Cannot decode image. Format not supported by OpenCV, Android Native, or PIL. Details: {str(e)}")
...
def extract_paths_from_skeleton(skeleton: np.ndarray) -> List[np.ndarray]:
    ...
    paths = []
    degrees = {node: len(adj) for node, adj in G_adj.items()}
    ...
    while True:
        ...
        while True:
            neighbors = [n for n in G_adj[curr] if frozenset((curr, n)) not in visited_edges]
            if not neighbors:
                break
            ...
            if len(neighbors) > 1 and len(path) > 1:
                ...
                for n in neighbors:
                    ...
                    if n_len > 0 and p_len > 0:
                        ...
                        if dot > best_dot:
                            best_dot, best_n = dot, n
                            
            visited_edges.add(frozenset((curr, best_n)))
            degrees[curr] -= 1
            degrees[best_n] -= 1
            curr = best_n
            path.append(curr)
...
def create_gcode_visualization(gcode_lines: List[str]) -> str:
    ...
    for line in gcode_lines:
        ...
        if line.startswith(("G0", "G1")):
            ...
            if pen_down:
                drawn_lines.append([(current_x, current_y), (new_x, new_y)])
            else:
                travel_lines.append([(current_x, current_y), (new_x, new_y)])
            ...
            if r > 1e-6:
                ...
                if pen_down:
                    drawn_lines.extend(segs)
                else:
                    travel_lines.extend(segs)
            current_x, current_y = x_end, y_end

    if travel_lines:
        lc_travel = LineCollection(travel_lines, color="red", linewidth=0.55, alpha=0.55)
        ax.add_collection(lc_travel)
    if drawn_lines:
        lc_drawn = LineCollection(drawn_lines, color="black", linewidth=1.0, alpha=1.0)
        ax.add_collection(lc_drawn)

    if drawn_lines or travel_lines:
        all_lines = drawn_lines + travel_lines
        x_coords = [pt[0] for seg in all_lines for pt in seg]
        y_coords = [pt[1] for seg in all_lines for pt in seg]
...
def get_gpu_name():
    try:
        import os
        egl = os.popen("getprop ro.hardware.egl").read().strip()
        platform = os.popen("getprop ro.board.platform").read().strip()
        board = os.popen("getprop ro.product.board").read().strip()
        hw = platform or board
        if egl == "adreno":
            return f"Adreno GPU ({hw})" if hw else "Adreno GPU"
        elif egl in ["mali", "meow"]:
            return f"Mali GPU ({hw})" if hw else "Mali GPU"
        elif hw:
            return f"GPU ({hw})"
        return "Unknown GPU"
    except Exception:
...
def request_root():
    try:
        import subprocess
        # Try to request root silently first
        subprocess.run(["su", "-c", "id"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL, timeout=2)
    except Exception:
        pass

# Call once on startup
import threading
threading.Thread(target=request_root, daemon=True).start()
...
def process_image_route():
    try:
        ...
        timings["file_loading"] = round(time.time() - t_start, 3)
        t_last = time.time()
        ...
        timings["auto_optimization"] = round(time.time() - t_last, 3)
        t_last = time.time()
        ...
        timings["processing_and_generation"] = round(time.time() - t_last, 3)
        t_last = time.time()
        ...
        timings["gcode_optimization"] = round(time.time() - t_last, 3)
        t_last = time.time()
        
        preview_image_base64 = create_gcode_visualization(gcode_lines)
        
        timings["preview_generation"] = round(time.time() - t_last, 3)
        timings["total_time"] = round(time.time() - t_start, 3)
        
        print(f"\n--- Operation Timers (CUDA: {use_cuda}) ---")
        ...
        return jsonify(
            {
                ...
                "cuda_used": use_cuda or cv2.ocl.haveOpenCL(),
