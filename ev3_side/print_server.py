...
def parse_motion_values(cmd, current_x, current_y):
    x = current_x
    y = current_y
    i = 0.0
    j = 0.0
...
def find_last_position(commands, start_line):
    ...
    for i in range(0, start_line):
        ...
        if cmd.startswith(("G0", "G1", "G2", "G3")):
            x, y, _, _ = parse_motion_values(cmd, x, y)
    return (x, y)
...
def run_print_job(plotter, commands, start_line, start_pen_is_down):
    ...
    if start_line >= total_lines:
        print("Bad resume line: {} / {}. Not marking job done.".format(start_line, total_lines))
        return

    if start_line > 0:
        print("Resuming print from current physical position.")
        last_x, last_y = find_last_position(commands, start_line)
        print("Logical G-code position is ({}, {}). Not moving there.".format(last_x, last_y))
        if hasattr(plotter, "set_logical_position"):
            plotter.set_logical_position(last_x, last_y)
    ...
    try:
        for i in range(start_line, total_lines):
            ...
            if cmd == "M300 S30":
                ...
                x, y, _, _ = parse_motion_values(cmd, plotter.current_pos[0], plotter.current_pos[1])
                plotter.move_to(x, y)
            elif cmd == "G28":
                plotter.pen_up()
                plotter.move_to(0, 0)
            elif cmd.startswith(("G21", "G90", "G92", "F")):
                print("Ignoring: {}".format(cmd))
            elif cmd.startswith("G2") or cmd.startswith("G3"):
                x, y, i, j = parse_motion_values(cmd, plotter.current_pos[0], plotter.current_pos[1])
                plotter.draw_arc(x, y, i, j, clockwise=cmd.startswith("G2"))
